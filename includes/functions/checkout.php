...
/* set session cookie if it haven't set by default */
...
/* save any value changes from checkout fields to be use to get non-logged-in user data
 * from https://www.businessbloomer.com/woocommerce-retain-checkout-data-reload/
 */

function shop_extra_save_checkout_values( $posted_data ) {
    parse_str( $posted_data, $output );
    WC()->session->set( 'checkout_data', $output );
}
add_action( 'woocommerce_checkout_update_order_review', 'ShopExtra\shop_extra_save_checkout_values', 9999 );
 
function shop_extra_get_saved_checkout( $value, $index ) {
...
add_filter( 'woocommerce_checkout_get_value', 'ShopExtra\shop_extra_get_saved_checkout', 9999, 2 );
 
function shop_extra_get_saved_ship_to_different( $checked ) {
    $data = \WC()->session->get( 'checkout_data' );
    if ( ! $data || empty( $data['ship_to_different_address'] ) ) return $checked;
    return true;
}
add_filter( 'woocommerce_ship_to_different_address_checked', 'ShopExtra\shop_extra_get_saved_ship_to_different' );

function render_checkout_button_html($value) {
	...
	*/
	...
	if ( !get_option('shop_extra_options')['shop_extra_product_button_enable'] || !get_option('shop_extra_options')['shop_extra_product_button_checkout_enable'] ) {
        return;
    }
	...
	if ( !empty(get_option('shop_extra_options')['shop_extra_product_button_message_before']) ) {
		$checkout_button_message_before = get_option('shop_extra_options')['shop_extra_product_button_message_before'];
		/* convert message line breaks to whatsapp format */
		$checkout_button_message_before = str_replace("\r\n", "%0D%0A", $checkout_button_message_before);
	} else {
		$checkout_button_message_before = 'Hi,%0D%0AI want to buy:%0D%0A';
	}
	
	if ( !empty(get_option('shop_extra_options')['shop_extra_product_button_message_after']) ) {
		$checkout_button_message_after = get_option('shop_extra_options')['shop_extra_product_button_message_after'];
		/* convert message line breaks to whatsapp format */
		$checkout_button_message_after = str_replace("\r\n", "%0D%0A", $checkout_button_message_after);
	} else {
		$checkout_button_message_after = 'Thank you!';
	}
	...
	if ( !empty(get_option('shop_extra_options')['shop_extra_product_button_message_payment']) ) {
		$payment_text = get_option('shop_extra_options')['shop_extra_product_button_message_payment'];
	} else {
	...
	foreach($items as $item ) {
		...
		$checkout_total =  wp_strip_all_tags( wc_price(WC()->cart->get_cart_contents_total()) );
		...
		if ( get_option('shop_extra_options')['shop_extra_product_button_price_enable'] ) {
    		...
    		$total = urlencode("\r\n");
    		$total .= ''.$total_text.': *'.$checkout_total.'*';
	...
	/* give line break before $checkout_button_message_after */
	if ( get_option('shop_extra_options')['shop_extra_product_button_url_enable'] && get_option('shop_extra_options')['shop_extra_product_button_price_enable'] ) {
       $total .= urlencode("\r\n");
    }
    elseif ( get_option('shop_extra_options')['shop_extra_product_button_url_enable'] && !get_option('shop_extra_options')['shop_extra_product_button_price_enable'] ) {
    ...
    if ( get_option('shop_extra_options')['shop_extra_product_button_price_enable'] ) {
        ...
        /* start get customer address info from checkout data session */
        
        $get_customer_data = \WC()->session->get( 'checkout_data' );
        ...
        if( !empty($city) && !empty($get_country) && !empty($get_state) ) {
            ...
            if ( is_user_logged_in() ) {
                ...
                $firstname = $get_customer_data['billing_first_name'];
            }
            if ( !get_option('shop_extra_options')['shop_extra_checkout_last_name_hide'] ) {
                if ( is_user_logged_in() ) {
                    $lastname = \WC()->customer->get_billing_last_name();
                } else {
                    $lastname = $get_customer_data['billing_last_name'];
                }
            }
            ...
            if ( is_user_logged_in() ) {
                ...
                $phone = $get_customer_data['billing_phone'];
            }
            if ( is_user_logged_in() ) {
                $email = \WC()->customer->get_billing_email();
            } else {
                $email = $get_customer_data['billing_email'];
            }
    	...
    	if( isset( $get_shiping ) ) {
        	...
        	foreach( WC()->session->get('shipping_for_package_0')['rates'] as $method_id => $rate ){
                ...
                if( $get_shiping[0] == $method_id ){
                    ...
                    */
                    //$rate_cost_incl_tax = \WC()->checkout->get_total_tax();
	...
	/* start get payment methods info from session */
	
	$use_payment_gateways = \WC()->payment_gateways->payment_gateways();
		
	if( isset( $use_payment_gateways ) ) {
	
		$get_payment = \WC()->session->get( 'chosen_payment_method' );
		
		if( isset( $get_payment ) ) {
		    
    		$payment_label = \WC()->payment_gateways()->payment_gateways()[ $get_payment ];
    		$shipping_details .= $payment_text;
    		$shipping_details .= ': *'.ucwords($payment_label->method_title).'*';
    		$shipping_details .= urlencode("\r\n\r\n");
    		
		}
	}
		
	/* end get payment methods info from session */
	
	/* replace spaces & line breaks to whatsapp format before rendered to html */
	
	$checkout_button_message_before = str_replace( " ", '+', $checkout_button_message_before );
	$checkout_button_message_before = str_replace( "\r\n", '%0D%0A', $checkout_button_message_before );
	...
	$checkout_button_message_after = str_replace( " ", '+', $checkout_button_message_after );
	$checkout_button_message_after = str_replace( "\r\n", '%0D%0A', $checkout_button_message_after );
	
	/* start rendering button html */
	
	$checkout_button_html = '<div class="shop-extra-checkout-button-container wc-proceed-to-checkout">';
	...
	foreach($names as $i => $name){
		...
		$checkout_button_html .= '
			<a id="shop-extra-checkout" rel="nofollow" target="_blank" href="https://'.esc_html($product_base_url).''.esc_attr($number).''.esc_html($product_base_text).''.esc_html($checkout_button_message_before).'%0D%0A'.esc_html($purchased_products).''.esc_html($total).''.esc_html($shipping_details).'%0D%0A'.esc_html($checkout_button_message_after).'%0D%0A" class="checkout-button button alt shop-extra-checkout-whatsapp-button">
				<img width="18" height="18" alt="'.esc_html($name).'" src="'.esc_url($image).'" class="shop-extra-checkout-icon">
				<span class="shop-extra-checkout-label">'.esc_html($name).'</span>
			</a>';
	...
	if ( get_option('shop_extra_options')['shop_extra_product_button_enable'] && get_option('shop_extra_options')['shop_extra_product_button_checkout_enable'] && !empty(get_option('shop_extra_options')['shop_extra_product_button_numbers']) && !empty(get_option('shop_extra_options')['shop_extra_product_button_names']) ) { 
	
	
	echo wp_kses_post($checkout_button_html);
 
	}
}

add_action( 'woocommerce_review_order_after_submit', 'ShopExtra\render_checkout_button_html', 10, 2 ); // after checkout button

//add_action( 'woocommerce_review_order_after_payment', 'ShopExtra\render_checkout_button_html', 10, 2 ); // after review order form